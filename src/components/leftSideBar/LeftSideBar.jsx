...
const LeftSideBar = () => {
  ...
  const currentPath = location.pathname.split("/")[1] || "default";
  ...
  const dashboards = [
    { label: "Default", icon: DashboardIcon, path: "default" },
    ...
    { label: "Online Courses", icon: OnlineCoursesIcon, path: "online-courses" },
  ];
  ...
  return (
    <section className="lg:grid lg:col-span-4 gap-y-4 text-sm font-light animate-fade px-4 py-5">
      ...
      <div className="pb-4 flex flex-col gap-1">
        ...
        {dashboards.map(({ label, icon: Icon, path }, i) => {
          ...
          return (
            <div
              key={i}
              onClick={() => navigate(`/${path === "default" ? "" : path}`)}
              className={`
