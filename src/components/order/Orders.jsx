...
import { PiArrowsDownUp, PiFunnelSimple, PiPlus } from "react-icons/pi";
...
const Orders = () => {
  ...
  return (
    <main className="grid gap-y-5 animate-fade text-text-primary">
      ...
      <div className="space-y-3 max-md:overflow-x-scroll">
        <div
          className={`bg-bg-surface-light flex items-center justify-between p-2 rounded-md`}
        >
          <div className="flex items-center gap-2">
            ...
            <span className="size-7 flex justify-center items-center hover:bg-bg-hover rounded-md">
              <SortIcon
                className="size-5 fill-current selected-icon"
        ...
        <div className="flex items-center gap-2 justify-center md:justify-end font-light">
          ...
          {[...Array(totalPages)].map((d, i) => (
            <p
              key={i}
              className={`${
                currentPage === i + 1 ? `bg-bg-hover` : ""
              } cursor-pointer text-sm font-light px-2 py-1 size-7 rounded-md flex items-center justify-center transition-transform hover:scale-105 duration-500 ease-in-out bg-bg-base hover:bg-bg-hover`}
